...
describe('component(slidey)', () => {
  ...
  beforeEach(angular.mock.inject(($compile, $rootScope) => {
    ...
    `)(scope);
    controller = component.controller('slidey');
    scope.$digest();
  }));

  describe('defaults', () => {
    beforeEach(angular.mock.inject(($compile, $rootScope) => {
      component = $compile('<slidey></slidey>')(scope);
      controller = component.controller('slidey');
      scope.$digest();
    }));
  ...
  describe('function(nested)', () => {
    ...
    beforeEach(angular.mock.inject(($compile, $rootScope) => {
      ...
      component = $compile(`
        <slidey opened="opened" content-width="{{contentWidth}}">
          <slidey opened="nestedOpened" content-width="{{nestedContentWidth}}">
      ...
      controller = component.controller('slidey');
      scope.$digest();
      nestedSlidey = component.find('slidey');
    }));
  ...
  describe('binding(reflowContainer)', () => {
    ...
    beforeEach(angular.mock.inject(($compile, $rootScope) => {
      modal = $compile(`
        <div id="modal">
          ...
          <slidey opened="opened" reflow-container="{{reflowContainer}}">
            <div style="height: 600px;">Some Content</div>
            <slidey opened="nestedOpened">
              <div style="height: 200px;">Some Content</div>
            </slidey>
          </slidey>
        </div>
      `)(scope);
      component = modal.find('slidey');
      modal = modal[0];
    ...
    afterEach(() => {
      ...
      sinon.restore(document, 'getElementById');
    });
